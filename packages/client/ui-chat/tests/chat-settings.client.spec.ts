...
import {
  CHAT_SETTINGS_NAMESPACE, DEFAULT_MARKDOWN_VIEW_MODE, DEFAULT_TRANSCRIPT_VIEW_MODE, apply,
} from '../src/index.ts'
...
describe('ui-chat Host settings', () => {
  it('registers, validates, and disposes the transcript-view namespace', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({
      ...
      markdownView: DEFAULT_MARKDOWN_VIEW_MODE,
    })
    ...
    await ctx.settings.update(ns, { markdownView: 'raw' })
    expect(ctx.settings.get(ns)).toEqual({ transcriptView: 'normal', markdownView: 'raw' })
    await expect(ctx.settings.update(ns, { markdownView: 'source' })).rejects.toThrow()
