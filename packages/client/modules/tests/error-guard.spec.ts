...
describe('frontend error guard script', () => {
  it('captures an uncaught error and reports it to the fallback intake', async () => {
    const h = makeHarness()
    h.dispatch('error', errorEvent({}))
    h.runFlush()
    const reports = await drain(h.sent)
    expect(reports).toHaveLength(1)
    expect(reports[0]!.url).toBe('/client-error')
  ...
  it('uses the decoupled self-rescue intake when configured', async () => {
    const h = makeHarness({ customIntake: 'http://127.0.0.1:18445/report' })
    h.dispatch('error', errorEvent({}))
    h.runFlush()
    const reports = await drain(h.sent)
    expect(reports[0]!.url).toBe('http://127.0.0.1:18445/report')
  })
  ...
  it('routes remote pages (mobile tunnel) to the same-origin rescue-intake proxy', async () => {
