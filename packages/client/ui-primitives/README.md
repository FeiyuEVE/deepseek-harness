...
| `MarkdownText`, `CodeBlock` | Untrusted GFM with TeX math, and highlighted code. `CodeBlock` accepts opt-in `lineNumbers`; copied source excludes the gutter, and `contentRef` exposes its stable source wrapper to an owner that uses it as a scrollport. `MarkdownText` accepts opt-in image-viewer labels, which turn each rendered image into a button opening a viewport-sized viewer. |
