...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and publishes the URL with the LAN snapshot', async () => {
    ...
    apply(ctx, new Config({ openBrowser: true, printUrl: true, surfaceContext: true, trustedHosts: ['lab.internal'], rescueIntakeUrl: '', ingestPageToken: '' }))
  ...
  it('publishes no readiness side effect when printing and browser opening are disabled', async () => {
    ...
    internals.openBrowser = openBrowser
    apply(ctx, new Config({ openBrowser: false, printUrl: false, surfaceContext: true, trustedHosts: [], rescueIntakeUrl: '', ingestPageToken: '' }))
    await ctx.plugin(SystemPrompt, { personaPrefix: '' })
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it('skips the surface context when disabled (the one-shot layer): no prompt section, no bash variables', async () => {
    ...
    apply(ctx, new Config({ openBrowser: false, printUrl: false, surfaceContext: false, trustedHosts: [], rescueIntakeUrl: '', ingestPageToken: '' }))
    await ctx.plugin(SystemPrompt, { personaPrefix: '' })
    await new Promise(resolve => setTimeout(resolve, 0))
    const assembly = await ctx.systemPrompt.assemble()
  ...
  it('prints the loopback-only URL line when no LAN snapshot exists', async () => {
    ...
    provideConnection(ctx)
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ openBrowser: false, printUrl: true, surfaceContext: true, trustedHosts: [], rescueIntakeUrl: '', ingestPageToken: '' }))
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it.each([
    ...
    const openBrowser = vi.fn(async () => {})
    internals.openBrowser = openBrowser
    apply(ctx, new Config({ openBrowser: true, printUrl: true, surfaceContext: false, trustedHosts: [], rescueIntakeUrl: '', ingestPageToken: '' }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('dsh web: http://127.0.0.1:4567/?token=test-token')
    expect(openBrowser).not.toHaveBeenCalled()
    await ctx.fiber.dispose()
  ...
  it('injects the decoupled rescue intake and ingest-page-token globals when configured', async () => {
    stageDist()
    const ctx = new Context()
    ctx.provide('webServer', fakeHttpServer().server)
    provideConnection(ctx)
    apply(ctx, new Config({
      ...
      ingestPageToken: 'page-token',
    }))
    const table: IndexInjection[] = []
    ctx.emit('webserver/index-inject', table)
    expect(table).toContainEqual({ kind: 'global', name: '__DSH_RESCUE_INTAKE__', value: 'http://127.0.0.1:18445/report' })
    expect(table).toContainEqual({ kind: 'global', name: '__DSH_INGEST_TOKEN__', value: 'page-token' })
    await new Promise(resolve => setTimeout(resolve, 0))
    await ctx.fiber.dispose()
  })

  it('omits both guarded-ingest globals when unconfigured', async () => {
    stageDist()
    const ctx = new Context()
    ctx.provide('webServer', fakeHttpServer().server)
    provideConnection(ctx)
    apply(ctx, new Config({
      openBrowser: false, printUrl: false, surfaceContext: false, trustedHosts: [], rescueIntakeUrl: '', ingestPageToken: '',
    }))
    const table: IndexInjection[] = []
    ctx.emit('webserver/index-inject', table)
    expect(table.some(entry => entry.kind === 'global')).toBe(false)
    await new Promise(resolve => setTimeout(resolve, 0))
    await ctx.fiber.dispose()
  })
  ...
  it('defers readiness publication until Loader settlement and drops it on failure or teardown', async () => {
    ...
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(settled, new Config({ openBrowser: true, printUrl: true, surfaceContext: true, trustedHosts: [], rescueIntakeUrl: '', ingestPageToken: '' }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).not.toHaveBeenCalled()
    expect(openBrowser).not.toHaveBeenCalled()
    release!()
    ...
    apply(failed, new Config({ openBrowser: true, printUrl: true, surfaceContext: true, trustedHosts: [], rescueIntakeUrl: '', ingestPageToken: '' }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).not.toHaveBeenCalled()
    expect(openBrowser).not.toHaveBeenCalled()
    ...
    apply(torn, new Config({ openBrowser: true, printUrl: true, surfaceContext: true, trustedHosts: [], rescueIntakeUrl: '', ingestPageToken: '' }))
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it.each([
    ...
    apply(ctx, new Config({ openBrowser: true, printUrl: false, surfaceContext: false, trustedHosts: [], rescueIntakeUrl: '', ingestPageToken: '' }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('dsh web: opening the default browser; pass --no-open to disable')
